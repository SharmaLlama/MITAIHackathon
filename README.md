...
An Apple iOS application demonstrating an edge-compatible, quantized acne lesion detection model powered by Core ML. This project builds on the open-source `acne-lds` repository and includes tools for model quantisation, comparison, prototyping YOLO training, and the final iOS app integration.
...
The objective of this project is to deploy an acne lesion detection system on Apple iOS devices by converting a quantized deep learning model into Core ML format. We leveraged the original [`acne-lds`](https://github.com/openface-io/acne-lds) repository for baseline architecture and data processing. Key features include:

* **Quantisation** of a modified ResNet model to reduce size and latency for edge deployment.
* **Core ML conversion** enabling on-device inference in an Xcode-based iOS application.
* **Comparison notebook** (`quantise_and_compare.ipynb`) for evaluating performance against ground truth, quantised and YOLO.
...
```text
├── quantise_and_compare/                # Comparison notebook
│   └── quantise_and_compare.ipynb       # Notebook comparing baseline, YOLO, and quantised pre-trained ResNet
├── prototyping/                         # Prototyping resources
│   └── yolo_training/                   # Scripts and configs for YOLO prototyping
├── app/SkinSnap/                        # iOS application
│   ├── SkinSnap.xcodeproj               # Xcode project file
│   ├── SkinSnap/                        # App source code and assets
│   ├── AcneClassQuantFin.mlpackage      # Final Core ML model converted from quantised pre-trained ResNet
│   └── README_ios.md                    # iOS-specific instructions
├── scripts/                             # Helper scripts for conversion and deployment
├── data/                                # Dataset download instructions or assets
└── README.md                            # This file
```
...
```text
https://universe.roboflow.com/andrei-dore-5lz05/acne04
```
...
4. Follow the notebook to:
   ...
   * Compare metrics (accuracy, size, latency)
...
Use this as a reference if you wish to retrain or fine-tune a YOLO model.
