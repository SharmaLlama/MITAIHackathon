...
struct HistoryView: View {
    ...
    func performEnhancedCorrelationAnalysis(
            ...
            // Define factors with their expected direction for skin health
            // isPositive: true means factor presence is expected to IMPROVE skin health
            // isPositive: false means factor presence is expected to WORSEN skin health
            let factorsToAnalyze: [(name: String, extractor: (LifestyleEntry) -> Bool, isPositive: Bool)] = [
                ("Dairy Consumption", { $0.dairyConsumed }, false),
                ("Sugar Consumption", { $0.sugarConsumed }, false),
                ("Alcohol Consumption", { $0.alcoholConsumed }, false),
                ("Fast Food", { $0.fastFoodConsumed }, false),
                ("High Protein Diet", { $0.highProteinMeal }, true),
                ("Sunscreen Use", { $0.usedSunscreen }, true),
                ("Makeup Use", { $0.usedMakeup }, false),
                ("Face Wash Use", { $0.usedFaceWash }, true),
                ("Moisturizer Use", { $0.usedMoisturizer }, true),
                ("Acne Treatment", { $0.usedAcneTreatment }, true),
                ("Fresh Pillowcase", { $0.changedPillowcase }, true),
                ("Travel", { $0.traveledRecently }, false),
                ("Climate Change", { $0.climateChange }, false)
            ]
            
            // Continuous factors with expected direction
            let continuousFactors: [(name: String, extractor: (LifestyleEntry) -> Double, threshold: Double, comparison: (Double, Double) -> Bool, isPositive: Bool)] = [
                ("Low Sleep", { $0.sleepHours }, 7.0, { $0 < $1 }, false), // Less than 7 hours (negative)
                ("High Sleep", { $0.sleepHours }, 8.0, { $0 >= $1 }, true), // 8 or more hours (positive)
                ("High Stress", { Double($0.stressLevel) }, 3.0, { $0 > $1 }, false), // Higher than 3 (negative)
                ("Low Water Intake", { Double($0.waterIntake) }, 6.0, { $0 < $1 }, false), // Less than 6 glasses (negative)
                ("High Water Intake", { Double($0.waterIntake) }, 8.0, { $0 >= $1 }, true), // 8+ glasses (positive)
                ("Many Fruits & Veggies", { Double($0.fruitsAndVeggiesServings) }, 4.0, { $0 >= $1 }, true), // 4+ servings (positive)
                ("High Mood", { Double($0.moodRating) }, 4.0, { $0 >= $1 }, true), // 4+ rating (positive)
                ("High Anxiety", { Double($0.anxietyLevel) }, 3.0, { $0 >= $1 }, false), // 3+ anxiety (negative)
                ("Outdoor Exposure", { $0.outdoorHours }, 2.0, { $0 > $1 }, false), // 2+ hours (potentially negative)
                ("High Sun Exposure", { Double($0.sunExposureLevel) }, 3.0, { $0 >= $1 }, false) // 3+ exposure (negative)
            ]
            ...
            for (factorName, factorExtractor, isPositive) in factorsToAnalyze {
                for timeWindow in timeWindows {
                    let result = analyzeFactorAcrossTimeWindow(
                        ...
                        isPositiveForHealth: isPositive
                    )
            ...
            for (factorName, factorExtractor, threshold, comparison, isPositive) in continuousFactors {
                for timeWindow in timeWindows {
            ...
            // Convert to HistoryCorrelationResult format with clear descriptions
            return topResults.map { result in
                // Determine description based on the nature of the factor and correlation
                let description = generateDescriptionForFactor(result)
        ...
        // Generate human-readable description for correlation results
        private func generateDescriptionForFactor(_ result: FactorAnalysisResult) -> String {
            let healthDifference = abs(Int(round(result.healthDifference)))
            
            if result.dataPoints < 3 {
                return "Not enough data to determine effect"
            }
            
            if abs(result.healthDifference) < 5 {
                return "No significant effect detected"
            }
            
            if result.impactType == .positive {
                return "\(result.factorName) associated with reduced skin health (by \(healthDifference) points)"
            } else if result.impactType == .negative {
                return "\(result.factorName) associated with better skin health (by \(healthDifference) points)"
            } else {
                return "No clear correlation detected"
            }
        }
        
        // Analyze a factor considering its expected effect (positive or negative for health)
        private func analyzeFactorAcrossTimeWindow(
            ...
            if factorActiveHealth.count < 2 || factorInactiveHealth.count < 2 {
                ...
                // Important: Interpret correlation based on expected effect
                // For factors where presence is positive (like moisturizer), positive correlation is good
                // For factors where presence is negative (like dairy), positive correlation is bad
                if isPositiveForHealth {
                    // Factor presence should improve health
                    if correlation > 0.15 {
                        impact = .positive
                    ...
                    // Factor presence should worsen health, so interpretation is reversed
                    if correlation > 0.15 {
                        impact = .negative // Factor expected to be bad actually correlates with better health
                    } else if correlation < -0.15 {
                        impact = .positive // Factor expected to be bad correlates with worse health as expected
                    } else {
                        impact = .neutral
                    }
                }
        ...
        // Similar fix for continuous factors
        private func analyzeContinuousFactorAcrossTimeWindow(
            ...
            // Same implementation as above but with continuous factor extraction
            // Similar logic to the binary factor analysis but adapted for continuous values
            let calendar = Calendar.current
